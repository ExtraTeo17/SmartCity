export const D_ZOOM = 15;
export const D_DECIMAL_PLACES = 5;

export const D_LAT = 52.23698;
export const D_LNG = 21.01766;
export const D_RAD = 600;

export const D_USE_FIXED_ROUTES = true;
export const D_USE_FIXED_TP = true;

export const D_START_TIME = new Date();
export const D_TIME_SCALE = 10;

export const D_GENERATE_CARS = true;
export const D_CARS_NUM = 1;
export const D_TEST_CAR = 2;
export const D_GENERATE_BATCHES_FOR_CARS = false;

export const D_GENERATE_BIKES = false;
export const D_BIKES_NUM = 8;
export const D_TEST_BIKE = 4;

export const D_GENERATE_PEDS = false;
export const D_PEDS_NUM = 20;
export const D_TEST_PED = 5;

export const D_GENERATE_TP = true;
export const D_TIME_BEFORE_TROUBLE = 5;

export const D_GENERATE_BUS_FAILURES = false;
export const D_DETECT_TRAFFIC_JAMS = false;

export const D_LIGHT_STRATEGY_ACTIVE = true;
export const D_EXTEND_LIGHT_TIME = 20;

export const D_STATION_STRATEGY_ACTIVE = true;
export const D_EXTEND_WAIT_TIME = 60;

export const D_TP_STRATEGY_ACTIVE = true;
export const D_TJ_STRATEGY_ACTIVE = false;
export const D_TRANSPORT_CHANGE_STRATEGY_ACTIVE = false;

export const D_HERE_MAP_STYLE = "hybrid.day";
